...
const local = {
    ...
    laneFilter: 'ALL',
    roleFilter: 'ALL',
    filterMode: 'lane', // 'lane' | 'role' — toggled via #filterModeToggle
    muted: false,
...
function wireFilters(){
    ...
    document.getElementById('filterModeToggle')?.addEventListener('click', ()=>{
        ...
        const laneBar = document.getElementById('laneFilterBar');
        const roleBar = document.getElementById('roleFilterBar');
        if(local.filterMode === 'lane'){
            local.filterMode = 'role';
            laneBar?.classList.add('hidden');
            roleBar?.classList.remove('hidden');
            if(toggle) toggle.textContent = 'ROLE FILTER ⇄';
        } else {
            local.filterMode = 'lane';
            roleBar?.classList.add('hidden');
            laneBar?.classList.remove('hidden');
            if(toggle) toggle.textContent = 'LANE FILTER ⇄';
        }
        renderHeroGrid();
    });
